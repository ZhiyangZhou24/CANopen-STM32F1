...
/**
  * @brief在总线bus_id上传输SDO帧
  * @param * d CAN对象数据结构的指针
  * @param whoami取2个值：SDO_CLIENT或SDO_SERVER
  * @param CliServNbr涉及客户端或服务器对象
  * @param data要传输的8个字节的数组
  * @return canSend（bus_id，＆m）或0xFF如果错误。
 */
...
/**
  * @ingroup sdo
  * @brief用于发送SDO请求帧以在索引和指示的subIndex处写入数据
  * @param * d指向CAN对象数据结构的指针
  * @param nodeId从站的节点ID
  * @param index指示索引
  * @param subIndex指示subIndex
  * @param计算在dictionnary中写入的字节数。
  * @param dataType（在objdictdef.h中定义）：将“visible_string”表示为字符串，将0表示为整数或实数或其他值。
  * @param * data指向数据的指针
  * @return
  *  - 成功后返回0。
  *  - 当没有sdo客户端与节点通信时返回0xFE。
  *  - 发生错误时返回0xFF。
 */
...
/**
  * @ingroup sdo
  * @brief用于发送SDO请求帧以写入远程节点字典。
  * @details必须在用户代码中定义的函数Callback在
  *交易结束。 （关于成功或中止）。
  * @param * d指向CAN对象数据结构的指针
  * @param nodeId从站的节点ID
  * @param index指示索引
 ...
 * @param * data指向数据的指针
 * @param回调回调函数
 * @param endianize当不为0时，数据被重新编码为网络字节顺序
 *为0时，数据未以机器本机格式化并复制
 * endianness
 * @param useBlockMode如果使用块模式传输，则为true
 * @return
 *  - 成功后返回0。
 *  - 发生错误时返回0xFF。
*/
